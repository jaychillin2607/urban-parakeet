...
class StatueDataset(Dataset):
    ...
    def __init__(
        ...
        if "class_id" not in self.annotations.columns:
            self.annotations["class_id"] = self.annotations["class"].map(config.data.class_map)
        ...
        logger.info(f"Loaded {len(self.image_files)} images with {len(self.annotations)} annotations")
    ...
    def _create_annotation_mapping(self) -> Dict[str, pd.DataFrame]:
        ...
        for filename in self.image_files:
            file_to_annotations[filename] = self.annotations[self.annotations["filename"] == filename]
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for _, row in image_annotations.iterrows():
                # Check if we need to update width/height (for images from statues_labels2.csv)
                if row["width"] == 0 and row["height"] == 0:
                    # Update dataframe in-place
                    idx_in_df = image_annotations.index.get_loc(row.name)
                    self.annotations.at[row.name, "width"] = width
                    self.annotations.at[row.name, "height"] = height
                    image_annotations.at[idx_in_df, "width"] = width
                    image_annotations.at[idx_in_df, "height"] = height
